...
\title{\texttt{badp}: Bayesian model averaging for dynamic panels \\
with weakly exogenous regressors$^{1}$}

\author{%
  Krzysztof Beck$^{2}$ \and
  Piotr Cukier$^{2}$ \and
  Marcin Dubel$^{2}$ \and
  Mariusz Szczepańczyk$^{2}$ \and
  Mateusz Wyszyński$^{3}$
}
...
\footnotetext[1]{This article was prepared within the research project “Bayesian simultaneous equations model averaging - theoretical development and R package,” funded by the Polish National Science Centre, on the basis of the decision No. DEC-2021/43/B/HS4/01745, $^{2}$Lazarski University, $^{3}$University of Warsaw}
%\setpapersize{A4}
% \VignetteIndexEntry{badp: Bayesian dynamic systems modelling}
\maketitle
\abstract{This manuscript introduces the \verb+badp+ package,
...
The \verb+badp+ package was developed to address this gap.
...
In addition to the key features developed by \citet{Moral+2016}, the \verb+badp+ package offers a wide range of additional functionalities.
...
The \verb+badp+ package provides users with graphical options for plotting prior and posterior model probabilities across model sizes and the model space.
...
Section \ref{using_bma} provides an overview of the \verb+badp+ functions related to performing Bayesian model averaging, calculating jointness measures, and presenting the estimation results.
...
\mathbb{E}(\beta_k|\text{data})=\sum_{j=1}^{2^K}\widehat{\beta_k}_{j} \cdot \mathbb{P}(M_{j}, \pi_{\beta_k} = 1 |\text{data})
\end{equation}

\noindent where $\widehat{\beta_k}_{j}$ is the value of the coefficient $\beta_k$ in model $j$.
...
because the probability $\mathbb{P}(M_{j}, \pi_{\beta_k} = 1 |\text{data})$ wil be zero anyway.

The posterior variance of the parameter $\beta_k$ is equal to:

\begin{equation}
\begin{split}
Var(\beta_k|\text{data}) =\ &\sum_{j=1}^{2^K}Var(\beta_{k,j}|\text{data},M_{j}) \cdot \mathbb{P}(M_{j}, \pi_{\beta_k} = 1 |\text{data})\\[1ex]
&+\sum_{j=1}^{2^K}\Bigl[\widehat{\beta}_{k,j}-\mathbb{E}(\beta_k|\text{data})\Bigr]^{2} \cdot \mathbb{P}(M_{j}, \pi_{\beta_k} = 1 |\text{data})
\end{split}
\end{equation}

\noindent where $Var(\beta_{k,j}|\text{data},M_{j})$ denotes the conditional variance of the coefficient $\beta_k$ in model $M_{j}$
...
SD(\beta_k | \text{data}) = \sqrt{ Var(\beta_k | \text{data}) }
\end{equation}
...
\begin{equation}
\mathbb{E}(\beta_k | \pi_{\beta_k}=1,\text{data})=\frac{\mathbb{E}(\beta_k|\text{data})}{\mathbb{P}(\pi_{\beta_k} = 1|\text{data})}.
\end{equation}
...
\begin{equation}
SD(\beta_k | \pi_{k}=1,\text{data}) = \sqrt{ Var(\beta_k | \pi_{k}=1,\text{data}) }
\end{equation}
...
\noindent The function returns a list with five named arguments
which are explained in the subsections below.
A progress bar is displayed to easily track the ongoing computation.

\indent Since the MLEs for the parameters are found through numerical optimization,
...
\indent As an alternative to the approach developed by \cite{Moral+2016}, the user may estimate the model space using the non-nested approach described at the end of Subsection~\ref{model_setup}. To do so, the user needs to set the parameter \verb+nested+ to \verb+FALSE+.
...
\noindent In the remainder of the manuscript, we use the results obtained under the nested approach. However, all functionalities in the package operate in the same way under the non-nested approach.
...
Hence, for users who want to quickly start practicing using the \verb+badp+,
...
\subsection{Visualizing model coefficients and posterior distribiutions}
...
There are two main options for visualizing the coefficient distributions. The first option uses a histogram. The \verb+coef_hist+ function provides the user with options for controlling the bin widths of the histogram (\verb+BW+, \verb+binW+, \verb+BN+, and \verb+num+).
...
One additional option available to the user with the \verb+coef_hist+ function is weighting coefficients by posterior model probabilities. This can be accomplished using the \verb+weight+ parameter, whose default value is set to \verb+NULL+. Setting \verb+weight+ to \verb+"binomial"+ or \verb+"beta"+ results in plotting histograms based on the binomial or binomial–beta model prior, respectively. For example, in the case of the binomial–beta model prior, it can be observed that the value of the posterior mean is mainly driven by the model characterized by the highest posterior model probability.
...
The \verb+posterior_dens+ allows the user to plot posterior distributions of coefficients. Similarly, to \verb+coef_hist+ function, \verb+posterior_dens+ returns a list containing a number of objects equal to the number of regressors plus one. The first object in the list is a graph of the posterior distribution for the lagged dependent variable, while the remaining objects are graphs of the posterior distribution for the other regressors. The user needs to specify whether to plot posterior distributions based on the binomial (\verb+prior = "binomial"+) or binomial-beta (\verb+prior = "beta"+) model prior, as well as whether to use regular (\verb+SE = "standard"+) or robust (\verb+SE = "robust"+) standard errors used in the calculation of posterior objects.
...
This manuscript introduces the \verb+badp+ package, which enables Bayesian model averaging for dynamic panels with weakly exogenous regressors — a methodology developed by \citet{Moral+2012,Moral+2013,Moral+2016}.
