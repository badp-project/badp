...
periods, four of the five available options are the same construction with
...
- `"curvature"`, with $\eta$ estimated from the data rather than fixed a
  priori, by the magnitude adjustment developed for misspecified and composite
  likelihoods [@Chandler+2007; @Ribatet+2012].
  When a likelihood overstates the information in the data, the observed
  information and the outer product of the entity-level scores disagree, and
  rescaling the log-likelihood restores the nominal calibration of the
  likelihood ratio. The realized value is returned in the `eta` element.
  This option requires models with fewer parameters than there are entities:
  the rate is $\dim(\theta)/\operatorname{tr}(H^{-1}J)$ with
  $J = \sum_{i=1}^{N} s_i s_i'$, which has rank at most $N$. `bma` warns when
  the condition fails, and an estimate above one is then a symptom of the
  deficiency rather than a meaningful value. It fails for the economic growth
  model space used here, whose models carry between 88 and 106 parameters
  against 73 countries.

The fifth option, `"uip"`, instead alters the penalty rather than the rate,
